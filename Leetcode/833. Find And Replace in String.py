...
class Solution:
    def findReplaceString(self, s: str, indices: List[int], sources: List[str], targets: List[str]) -> str:
        l = sorted([(indices[i], sources[i], targets[i]) for i in range(len(indices))])
        i = 0
        j = 0
        res = []
        while i < len(s):
            if j < len(l):
                index, source, target = l[j]
                if i == index and s[index:index+len(source)] == source:
                    res.append(target)
                    i += len(source)
                    j += 1
                    continue
...
class TestSolution(unittest.TestCase):
    ...
    def assert_function(self, first, second):
        self.assertEqual(Solution().findReplaceString(*first), second,
