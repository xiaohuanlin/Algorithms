...
class Solution1765 {
    public int[][] highestPeak(int[][] isWater) {
        ...
        while (!waters.isEmpty()) {
            int[] position = waters.poll();
            int x = position[0];
            int y = position[1];

            // add other positions into queue
            for (int[] direction: new int[][] {{1, 0}, {0, 1}, {-1, 0}, {0, -1}}) {
                int nextX = x + direction[0];
                int nextY = y + direction[1];
                int[] pos = {nextX, nextY};
                if (nextX >= 0 && nextX < row && nextY >= 0 && nextY < col && isWater[nextX][nextY] == -1) {
...
class Driver1765 {
    public static void main(String[] args) {
        ...
        System.out.println(new Solution1765().highestPeak(isWater));
    }
}